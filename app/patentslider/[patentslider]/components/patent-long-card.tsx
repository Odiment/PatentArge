...
const PatentLongCard: React.FC<PatentCard> = ({
  data,
  bilgiler,
  patent_id,
  patentResimler,
  ...
  var ilgiliProductResimler = productResimler.reduce(
    (ilgiliProductResimler, thing) => {
      if (thing.patent_id.includes(`${patent_id}`)) {
        ilgiliProductResimler.push(thing)
      }
      return ilgiliProductResimler
    },
    []
  )
  ...
  var ilgiliProductRemoteResimler = productRemoteResimler.reduce(
    (ilgiliProductRemoteResimler, thing) => {
      if (thing.patent_id.includes(`${patent_id}`)) {
        ilgiliProductRemoteResimler.push(thing)
      }
      return ilgiliProductRemoteResimler
    },
    []
  )
  ...
  const [product_url, setProductUrl] = useState<
    Patentler['product_figure_url']
  >(product_resimler_url[currentProductIndex])
  ...
  const [product_remote_url, setProductRemoteUrl] = useState<
    Patentler['product_remote_figure_url']
  >(product_remote_resimler_url[currentProductIndex])
  ...
  const prevProductSlide = () => {
    ...
    const newIndex = isFirstSlide
      ? product_resimler_url.length - 1
      : currentProductIndex - 1
    setCurrentProductIndex(newIndex)
    setProductUrl(product_resimler_url[currentProductIndex])
    setProductRemoteUrl(product_remote_resimler_url[currentProductIndex])
  }

  const nextProductSlide = () => {
    const isLastSlide = currentProductIndex === product_resimler_url.length - 1
