...
const MarkaCardTek: React.FC<MarkaCardTek> = ({ data }) => {
  ...
  const [logo_url, setLogoUrl] = useState<string>(data && data[0]?.logo_url!);
  const [tp_logo_url, setTPLogoUrl] = useState<string>(data[0]?.tp_logo_url!);
  ...
  async function updateMarkaLogo({ logo_url }: { logo_url: string | null }) {
    try {
      setLoading(true);
